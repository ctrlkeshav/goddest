...
function registerHandlers (ipcMain, db, dbPath, documentsPath, backupPath) {
  ...
  ipcMain.handle('backup:create', async () => {
    try {
      ...
      // Flush the in-memory database to disk before archiving
      db.saveToDisk()
  ...
  ipcMain.handle('backup:restore', async (_, filePath) => {
    try {
      ...
      // Close DB (flushes + frees WASM memory) before overwriting the file
      db.close()
      ...
      if (fs.existsSync(restoredDocs)) {
        ...
        fs.readdirSync(restoredDocs).forEach(f => {
          fs.copyFileSync(
            path.join(restoredDocs, f),
            path.join(documentsPath, f)
          )
        })
      }
