...
export default function DocumentsPage() {
  ...
  const handleBrowse = async () => {
    // Browser file picker — openFileDialog mock triggers <input type=file>
    ...
    if (!result.canceled && result.filePaths[0]) {
      const fname = result.filePaths[0].split(/[\\/]/).pop()
      setForm(p => ({
        ...p,
        source_path: result.filePaths[0],
        original_filename: fname,
        document_name: p.document_name || fname,
        // Approximate file size for preview
        file_size: result.files?.[0]?.size || 102400
      }))
    }
  }
            ...
            <div key={d.id} className="card" style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 6, marginTop: 2 }}>
                <button className="btn btn-secondary btn-sm" style={{ flex: 1 }}
                  onClick={() => toast('📄 File preview not available in browser mode', { icon: 'ℹ️' })}>
                  👁 Open
                </button>
